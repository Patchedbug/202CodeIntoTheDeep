...
public class BasicTeleOp extends LinearOpMode {
    ...
    static class RobotHardware{
        ...
        public void grabberControl(boolean lbump, boolean rbump) {
            if (lbump) Grab.setPosition(0.25 + ClawOffset);
            else if (rbump) Grab.setPosition(0.5 + ClawOffset);
        }
        ...
        public void wristControl(boolean twistto90, boolean twistto0, boolean wristup, boolean wristdown, boolean wristmid ) {
            ...
            if(wristup&&!dpad&&pivot<2){
                dpad=true;
                pivot=pivot+1;
            }
            else{dpad=false;}
            if(wristdown&&!dpad&&pivot>0){
                dpad=true;
                pivot=pivot-1;
            }
            else{dpad=false;}

            if(twistto90&&!aB&&rotate<2){
                aB=true;
                rotate=rotate+1;
            }
            else{aB=false;}
            if(twistto0&&!aB&&rotate>0){
                aB=true;
                rotate=rotate-1;
            }
            else{aB=false;}
            ...
            Wrist0.setPosition(pos0);
            Wrist1.setPosition(pos1);
